...
fn run_contract_with_tx_sequence(
    ...
    let output0 = TransactionOutput {
        value: output0_value,
        script_public_key: ScriptPublicKey::new(0, output0_script.into()),
        covenant: None,
    };
    let output1 = TransactionOutput {
        value: output1_value,
        script_public_key: ScriptPublicKey::new(0, output1_script.into()),
        covenant: None,
    };
    ...
    let utxo_entry = UtxoEntry::new(
        input_value,
        ScriptPublicKey::new(0, script.clone().into()),
        0,
        tx.is_coinbase(),
        None,
    );
...
fn run_contract_with_outputs(
    ...
    let tx_outputs = outputs
        ...
        .map(|(value, script)| TransactionOutput {
            value,
            script_public_key: ScriptPublicKey::new(0, script.into()),
            covenant: None,
        })
        .collect::<Vec<_>>();
...
fn compiles_announcement_example_and_verifies() {
    ...
    assert!(
        result.is_ok(),
        "announcement small change failed: {}",
        result.unwrap_err()
    );
}
...
fn compiles_constant_budget_example_and_verifies() {
    ...
    assert!(
        result.is_ok(),
        "constant_budget if branch failed: {}",
        result.unwrap_err()
    );
    ...
    assert!(
        result.is_ok(),
        "constant_budget else branch failed: {}",
        result.unwrap_err()
    );
}
...
fn compiles_for_loop_example_and_verifies() {
    ...
    let result =
        run_contract_with_outputs(compiled.script.clone(), outputs, input_value, sigscript, 0);
    assert!(
        result.is_ok(),
        "for_loop example failed: {}",
        result.unwrap_err()
    );
    ...
    let input_value = 10_000u64;
    let outputs = vec![
        (1000u64, output0_script),
        (1001u64, output1_script),
        (1002u64, output2_script),
    ];
    let result = run_contract_with_outputs(compiled.script, outputs, input_value, sigscript, 0);
    assert!(
        result.is_err(),
        "for_loop with too few outputs should error"
    );
}
...
fn compiles_mecenas_example_and_verifies() {
    ...
    let mut funder_hash = blake2b_simd::Params::new()
        .hash_length(32)
        .to_state()
        .update(funder_pk.as_slice())
        .finalize()
        .as_bytes()
        .to_vec();
    ...
    assert!(
        result.is_ok(),
        "mecenas small change failed: {}",
        result.unwrap_err()
    );

    let input = TransactionInput {
        previous_outpoint: TransactionOutpoint {
            transaction_id: TransactionId::from_bytes([15u8; 32]),
            index: 0,
        },
        signature_script: vec![],
        sequence: 0,
...
fn compiles_mecenas_locktime_example_and_verifies() {
    ...
    assert!(
        result.is_ok(),
        "mecenas_locktime example failed: {}",
        result.unwrap_err()
    );
    ...
    assert!(
        result.is_ok(),
        "mecenas_locktime small change failed: {}",
        result.unwrap_err()
    );

    let input = TransactionInput {
        previous_outpoint: TransactionOutpoint {
            transaction_id: TransactionId::from_bytes([16u8; 32]),
            index: 0,
        },
        signature_script: vec![],
        sequence: 0,
    ...
    assert!(
        result.is_ok(),
        "mecenas_locktime reclaim failed: {}",
        result.unwrap_err()
    );
}
...
fn compiles_p2pkh_example_and_verifies() {
    ...
    let mut pkh = blake2b_simd::Params::new()
        .hash_length(32)
        .to_state()
        .update(pubkey_bytes.as_slice())
        .finalize()
        .as_bytes()
        .to_vec();
    pkh.truncate(20);
    ...
    let input = TransactionInput {
        previous_outpoint: TransactionOutpoint {
            transaction_id: TransactionId::from_bytes([5u8; 32]),
            index: 0,
        },
        signature_script: vec![],
        sequence: 0,
...
fn compiles_transfer_with_timeout_and_verifies() {
    ...
    let input = TransactionInput {
        previous_outpoint: TransactionOutpoint {
            transaction_id: TransactionId::from_bytes([6u8; 32]),
            index: 0,
        },
        signature_script: vec![],
        sequence: 0,
    ...
    assert!(
        result.is_ok(),
        "transfer_with_timeout transfer failed: {}",
        result.unwrap_err()
    );
    ...
    assert!(
        result.is_ok(),
        "transfer_with_timeout timeout failed: {}",
        result.unwrap_err()
    );
}
...
fn compiles_covenant_escrow_example_and_verifies() {
    ...
    let mut arbiter_hash = blake2b_simd::Params::new()
        .hash_length(32)
        .to_state()
        .update(arbiter_pk.as_slice())
        .finalize()
        .as_bytes()
        .to_vec();
    ...
    let input = TransactionInput {
        previous_outpoint: TransactionOutpoint {
            transaction_id: TransactionId::from_bytes([10u8; 32]),
            index: 0,
        },
        signature_script: vec![],
        sequence: 0,
    ...
    assert!(
        result.is_ok(),
        "covenant escrow example failed: {}",
        result.unwrap_err()
    );
}
...
fn compiles_covenant_last_will_and_verifies() {
    ...
    let input = TransactionInput {
        previous_outpoint: TransactionOutpoint {
            transaction_id: TransactionId::from_bytes([12u8; 32]),
            index: 0,
        },
        signature_script: vec![],
        sequence: 180,
    ...
    assert!(
        result.is_ok(),
        "covenant last will inherit failed: {}",
        result.unwrap_err()
    );

    let input = TransactionInput {
        previous_outpoint: TransactionOutpoint {
            transaction_id: TransactionId::from_bytes([13u8; 32]),
            index: 0,
        },
        signature_script: vec![],
        sequence: 0,
    ...
    assert!(
        result.is_ok(),
        "covenant last will cold failed: {}",
        result.unwrap_err()
    );
    ...
    let input = TransactionInput {
        previous_outpoint: TransactionOutpoint {
            transaction_id: TransactionId::from_bytes([14u8; 32]),
            index: 0,
        },
        signature_script: vec![],
        sequence: 0,
    ...
    assert!(
        result.is_ok(),
        "covenant last will refresh failed: {}",
        result.unwrap_err()
    );
}
...
fn compiles_covenant_mecenas_example_and_verifies() {
    ...
    assert!(
        result.is_ok(),
        "covenant mecenas example failed: {}",
        result.unwrap_err()
    );
    // Test receive() with changeValue <= pledge + minerFee (if branch).
    let mut sigscript = ScriptBuilder::new();
    ...
    assert!(
        result.is_ok(),
        "covenant mecenas small change failed: {}",
        result.unwrap_err()
    );

    let input = TransactionInput {
        previous_outpoint: TransactionOutpoint {
            transaction_id: TransactionId::from_bytes([17u8; 32]),
            index: 0,
        },
        signature_script: vec![],
        sequence: 0,
    ...
    assert!(
        result.is_ok(),
        "covenant mecenas reclaim failed: {}",
        result.unwrap_err()
    );
}
